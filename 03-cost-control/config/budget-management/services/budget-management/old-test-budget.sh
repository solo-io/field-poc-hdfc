...
BUDGET_SERVICE="${BUDGET_SERVICE:-quota-management}"
...
# Start port forwarding to quota-management service
...
# Make API request to quota-management
budget_api() {
  local method="$1"
  local endpoint="$2"
  local data="${3:-}"
...
# Flush Prometheus metrics by restarting the quota-management deployment
flush_prometheus() {
  log_header "Flushing Prometheus Metrics"

  log_info "Restarting quota-management deployment to reset metrics..."
